 ...
 * @since  1.6
 */
class AuthorModelArticles extends JModelList
{
	/**
	 * Category items data
	 *
	...
	protected $_categorychildren = null;
	...
	protected function populateState($ordering = 'ordering', $direction = 'ASC')
	{
		...
		$value = $app->input->get('author_id', 0, 'uint');
		...
		if ((!$params->get('show_noauth')) || (!JComponentHelper::getParams('com_content')->get('show_noauth'))) // todo: change this params
		{
	...
	public function getItems()
	{
		...
		$globalParams = JComponentHelper::getParams('com_content', true); // todo, change this into com_author
	...
	public function getCategory()
	{
		...
		{
			$categories = JCategories::getInstance('Content', ['countItems'=>0]);
			$this->_category = $categories->get('root');			// Compute selected asset permissions.
			if (is_object($this->_category))
			{
				$user  = JFactory::getUser();
				$asset = 'com_content.category.' . $this->_category->id;

				// Check general create permission.
				if ($user->authorise('core.create', $asset))
				{
					$this->_category->getParams()->set('access-create', true);
				}

				// TODO: Why aren't we lazy loading the children and siblings?
				$this->_categorychildren = $this->_category->getChildren();
				$this->_categoryparent = false;

				if ($this->_category->getParent())
				{
					$this->_categoryparent = $this->_category->getParent();
				}

				//$this->_rightsibling = $this->_category->getSibling();
				//$this->_leftsibling = $this->_category->getSibling(false);
			}
			else
			{
				$this->_categorychildren = false;
				$this->_categoryparent = false;
			}
		}
	...
	public function &getCategoryChildren()
	{
		...
		if ($this->_categorychildren)
		{
			...
			{
				$this->_categorychildren = ArrayHelper::sortObjects($this->_categorychildren, 'title', ($orderByPri === 'alpha') ? 1 : (-1));
			}
		}

		return $this->_categorychildren;
	}
